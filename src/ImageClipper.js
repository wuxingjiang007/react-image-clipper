...
export default class ImageClipper extends Component {
  ...
  // 设置裁剪框配置
  setClipViewBoxImg = () => {
    const { imgContainer, clipBox, clipperContainer, clipperBox } = this;
    ...
    let { width, height } = clipperContainer.getBoundingClientRect();
    ...
    clipperBox.style.width = `${imgWidth}px`
    clipperBox.style.height = `${imgHeight}px`
  ...
  getClipData = () => {
    ...
    this.setState({
      ...
      this.onChange(this.state.clipData)
    });
  }
  ...
  onChange = (clipData) => {
    typeof this.props.onChange === 'function' && this.props.onChange(clipData)
  }
  ...
  render() {
    ...
    return (
      <div
        ...
        className={`${styles.ImageClipperContainer} ${containerClass || styles.defaultclipperContainer}`}>

        {target ?
          <div ref={(node) => this.clipperBox = node}
            className={styles.clipperBox}>
